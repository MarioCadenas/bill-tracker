...
const NavBar = ({ categories, showAddCategory, setNewActiveCategory, activeCategory }) => {
  ...
  const listStyle = "p-4 inline bg-grey-lighter hover:bg-grey-light uppercase font-black cursor-pointer";

  return (
    <ul className="list-reset inline flex justify-center boder-b-4 mb-0">
      <li onClick={() => setActiveCategory('') } className={listStyle}>All</li>
      {
        categories
